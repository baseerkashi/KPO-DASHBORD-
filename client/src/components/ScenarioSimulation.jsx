...
export default function ScenarioSimulation({ financials, risk, rowsCount }) {
  ...
  return (
    <div className="glass-panel p-6 rounded-2xl flex flex-col gap-8 xl:flex-row">
        ...
        <div className="space-y-5">
          <div className="space-y-2">
            <div className="flex justify-between text-sm">
              <label className="font-medium text-emerald-400">Revenue Adjustment</label>
              <span className="text-emerald-100">{revAdj >= 0 ? "+" : ""}{revAdj}%</span>
            </div>
            <input 
          ...
          <div className="space-y-2">
            <div className="flex justify-between text-sm">
              <label className="font-medium text-rose-400">Expenses Adjustment</label>
              <span className="text-rose-100">{expAdj >= 0 ? "+" : ""}{expAdj}%</span>
            </div>
            <input 
      ...
      <div className="w-px bg-white/10 hidden xl:block"></div>

      {/* Recalculated Output & AI */}
      <div className="flex-1 space-y-6 flex flex-col justify-between">
        <div className="flex justify-between items-start mb-2">
           <h4 className="text-sm font-semibold uppercase text-slate-400">Before vs After</h4>
          ...
          <table className="w-full text-sm text-left">
            ...
            <tbody className="divide-y divide-white/5">
              <tr className="bg-slate-900/30">
                <td className="px-4 py-3 text-slate-300">Avg Profit/mo</td>
                <td className={`px-4 py-3 border-l border-white/5 font-mono ${simParams.avgRev - simParams.avgExp >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>
                  ${Math.round(simParams.avgRev - simParams.avgExp).toLocaleString()}
                </td>
                <td className={`px-4 py-3 border-l border-white/5 font-mono font-bold ${simParams.simMonthlyProfit >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>
                  ${Math.round(simParams.simMonthlyProfit).toLocaleString()}
                </td>
              </tr>
              <tr className="bg-slate-900/30">
                <td className="px-4 py-3 text-slate-300">Runway</td>
        ...
        <div className="bg-gradient-to-br from-indigo-900/40 to-slate-900/60 p-4 rounded-xl border border-indigo-500/20 shadow-[0_0_15px_rgba(99,102,241,0.1)]">
          <label className="text-xs font-semibold text-indigo-300 uppercase mb-2 block">AI Scenario Consultant</label>
          <div className="flex gap-2">
            <input 
              type="text" 
              placeholder="e.g. Is this safe if market slows down?"
              className="flex-1 bg-black/30 border border-white/10 rounded-lg px-3 py-2 text-sm text-white placeholder-slate-500 outline-none focus:border-indigo-500/50 transition-colors"
              value={prompt}
            ...
            <button 
              ...
              className="bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 text-white px-4 py-2 rounded-lg text-sm font-medium transition-all min-w-[80px]"
            >
          ...
          {aiResponse && (
            <div className="mt-4 text-sm text-slate-300 leading-relaxed bg-black/20 p-3 rounded-lg border border-indigo-500/10 whitespace-pre-line">
              <span className="text-indigo-400 font-bold mr-1">AI:</span> {aiResponse}
            </div>
          )}
        </div>
      </div>
